...
void GPIO_CfgPins (GPIO_PinCfg_type * PtrArrPins, uint8 CopyNumPins)
{
	...
	for (i=0;i<CopyNumPins;i++)
	{
		...
		switch (PtrArrPins[i].PinPuPd)
		{
			...
			/*Case of alternative function*/
			default:
				return;  /*To be added : return error*/
		}
	}
...
{
	//uint32 * x =(volatile uint32*)PORTsPtrArr[PortId] +  (volatile uint32*)0x3FC  ;
	uint32 * x = ((volatile uint32*)(((uint32)PORTsPtrArr[PortId] - (uint32)PERI_BIT_BAND_REGION_ST + 0x3FC ) * 32 + PinIndix * 4 + (uint32)PERI_BIT_BAND_ALIAS_ST ));
	*x = 1;
